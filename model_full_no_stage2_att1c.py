...
class Deraining(nn.Module):
    def __init__(self,args):
        ...
        # self.ats_feature = SCA_feature(in_channel=3, out_channel=3)
    ...
    def forward(self, x, kpts):
        ...
        # return out_SR, out_combine, w0, w1, w2, add_residual, mul_residual
...
class channel_attention(nn.Module):
    def __init__(self, in_channels = 15):
        ...
        self.relu = nn.LeakyReLU(0.2, True)
        sequence1 = [
            nn.Conv2d(in_channels=in_channels//3, out_channels=128, kernel_size=1),
            ...
            nn.Conv2d(in_channels = 32, out_channels = in_channels//3, kernel_size = 1),
            # nn.Softmax2d()
        ]
        ...
        sequence2 = [
            ...
            nn.Conv2d(in_channels=in_channels//3, out_channels=1, kernel_size=1),
            nn.LeakyReLU(0.2, True),
            # nn.Conv2d(in_channels=in_channels//4, out_channels=in_channels//3, kernel_size=1),
            nn.Conv2d(in_channels=1, out_channels=in_channels//3, kernel_size=1),
            nn.Sigmoid()
        ]
        self.model2 = nn.Sequential(*sequence2)
    def forward(self, x):
        x0 = self.relu(self.conv(x[:,0:3,:,:]))
        x1 = self.relu(self.conv(x[:,3:6,:,:]))
        x2 = self.relu(self.conv(x[:,6:9,:,:]))
        x123 = torch.cat((x0,x1,x2), dim=1)
        x123  = self.model1(x123)
        y = self.model2(x123)
        out = x123 * y
        # out0 = out[:,0:3,:,:]
        # out1 = out[:,3:6,:,:]
