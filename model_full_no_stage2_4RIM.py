...
class add_layer(nn.Module):
    def __init__(self, pretrained=256, num_chan=64):
        ...
        self.add_blk1 = add_block(in_channels=512, out_channels=512)
        self.add_blk2 = add_block(in_channels=512, out_channels=256)
        self.add_blk3 = add_block(in_channels=256, out_channels=128)
        self.add_blk4 = add_block(in_channels=128, out_channels=num_chan)
    ...
    def forward(self, x):
        ...
        add4 = self.add_blk4(add3)
        # add3 = operator + add1
        conv = self.relu(self.conv2(add4))
        out = (self.conv3(conv))
        return out
...
class mul_layer(nn.Module):
    def __init__(self, num_pretrained=256, num_chan=64):
        ...
        # self.conv1 = nn.Conv2d(3, num_chan, kernel_size=3, padding=1)
        self.mul_blk1 = mul_block(in_channels=512, out_channels=512)
        self.mul_blk2 = mul_block(in_channels=512, out_channels=256)
        self.mul_blk3 = mul_block(in_channels=256, out_channels=128)
        self.mul_blk4 = mul_block(in_channels=128, out_channels=num_chan)
    ...
    def forward(self, x):
        ...
        mul4 = self.mul_blk4(mul3)
        conv = self.relu(self.conv2(mul4))
