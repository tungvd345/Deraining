...
class add_layer(nn.Module):
    def __init__(self, pretrained=512, num_chan=64):
        super(add_layer, self).__init__()
        # self.conv1 = nn.Conv2d(3, num_chan, kernel_size=3, padding=1)
        self.add_blk1 = add_block(in_channels=pretrained, out_channels=256)
        self.add_blk2 = add_block(in_channels=256, out_channels=128)
        self.add_blk3 = add_block(in_channels=128, out_channels=num_chan)
...
class mul_layer(nn.Module):
    def __init__(self, num_pretrained=512, num_chan=64):
        super(mul_layer, self).__init__()
        # self.conv1 = nn.Conv2d(3, num_chan, kernel_size=3, padding=1)
        self.mul_blk1 = mul_block(in_channels=num_pretrained, out_channels=256)
        self.mul_blk2 = mul_block(in_channels=256, out_channels=128)
        self.mul_blk3 = mul_block(in_channels=128, out_channels=num_chan)
        self.conv2 = nn.Conv2d(num_chan, 32, kernel_size=1)
        self.conv3 = nn.Conv2d(32, 3, kernel_size=1)
        self.relu = nn.LeakyReLU(0.2, True)
        self.sigmoid = nn.Sigmoid()
